...
| 4299 | VitalsComponentClientFacet_ClientSyncDeathRecap | u8 length, the killer's vitals name key (`@Invasion_Spearman_VitalsName`, English through the name book's display column) or, for a player, the character name followed by `24` + the 36-char uuid string; then u32, u16, f32 last hit (257.0), f32 the player's max health (9237.0), 1.0 multipliers |
...
| 839 | WarboardComponentClientFacet_OnUpdateWarboardStats | u16 version, u8 local index, local row (LE prefix-varint mask + values), two team blocks of u8 count + (u8 index, mask, values); running totals, bits in [outpost-rush.md](outpost-rush.md); `decode_opr.py` |
| 3530 / 2968 | OnUpdateFullWarboardManifest / OnUpdateWarboardManifest | full: per team u16 count, (05 + uuid) x count, u8 index x count, u16 0. Incremental: per team u16 added, entries, indices, u16 removed |
