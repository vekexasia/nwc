...
class Datagram:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "error":               self.error,
            "carrier_error":       self.carrier_error,
        }
...
def _parse_carrier_messages(buf: bytes) -> tuple[list[CarrierMessage], bytes, str | None]:
    """Parse Carrier frames and retain any unparsed suffix as a trailer."""
    ...
    while i < len(buf):
        ...
        if len(buf) - i < 3:
            return msgs, buf[frame_start:], "truncated carrier header"
        flags = buf[i]
        i += 1
        if flags & ~known_flags:
            return (msgs, buf[frame_start:],
                    f"unknown carrier flags 0x{flags & ~known_flags:02x}")
        data_size = int.from_bytes(buf[i:i+2], "big")
        i += 2
        m = CarrierMessage(flags=flags, data_size=data_size)
        if flags & MF_DATA_CHANNEL:
            if len(buf) - i < 1:
                return msgs, buf[frame_start:], "truncated carrier channel"
            m.channel = buf[i]
            i += 1
        if flags & MF_CHUNKS:
            if len(buf) - i < 2:
                return msgs, buf[frame_start:], "truncated carrier chunk count"
            m.num_chunks = int.from_bytes(buf[i:i+2], "big")
            i += 2
        # seq: written when MF_SQUENTIAL_ID is NOT set or this is the first
        # message in the datagram; otherwise derive from the prior frame.
        if is_first or not (flags & MF_SQUENTIAL_ID):
            if len(buf) - i < 2:
                return msgs, buf[frame_start:], "truncated carrier sequence"
            m.seq = int.from_bytes(buf[i:i+2], "big")
            i += 2
            previous_seq = m.seq
        else:
...
def parse_carrier_messages(buf: bytes, is_first_seq_assumed: bool = True
    ...
    """Parse Carrier frames, returning messages and an unparsed trailer."""
    messages, trailer, _ = _parse_carrier_messages(buf)
...
def decode_record(idx: int, dir_byte: int, ts_ms: int, ssl_ptr: int,
    ...
    if d.compressed:
        ...
        msgs, trailer, parse_error = _parse_carrier_messages(decoded)
    else:
        d.decompressed_len = len(body)
        msgs, trailer, parse_error = _parse_carrier_messages(body)
    if parse_error:
        d.carrier_error = parse_error
    d.messages = msgs
    d.trailer = trailer
    return d
